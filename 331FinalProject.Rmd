...
# dioxin heatmap
r_dioxin <- cor(POP_dioxin_data)
heatmaply_cor(
  r_dioxin,
  k_col = 2,
  k_row = 2
)
# interaction in dioxin
f <- as.formula((paste("length", paste("(", paste(POP_dioxin, collapse = " + "), ")^2"), sep = " ~")))
m <- lm(f, data = pollutants)
summary(m)


# Weiwei's work for furan
POP_furan = c("POP_furan1", "POP_furan2","POP_furan3","POP_furan4")
POP_furan_data <- pollutants [, POP_furan]

# furan heatmap
r_furan <- cor(POP_furan_data)
heatmaply_cor(
  r_furan,
  k_col = 2,
  k_row = 2
...
f <- as.formula((paste("length", paste("(", paste(POP_furan, collapse = " + "), ")^2"), sep = " ~")))
m <- lm(f, data = pollutants)
summary(m)
```
