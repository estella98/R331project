...
Now to assess whether any covariate has a non-linear relationship with the outcome in the multiple linear regression model, we used added-variable plots(avPlot), as shown in appendix \ref{AvPlots}. The plots isolate the relationship between the outcome and each of the covariates after adjusting for the other covariates. If the plot of the outcome versus a covariate x has a nonlinear shape, it may indicate a regression model with a higher power of this variable, for example, $x^2$. With the given data, we see that all plots have a linear shape, thus the outcome is expected to have a linear relationship with all of the given covariates. Therefore, the models constructed in this report do not consider non-linear terms.
...
### Investigate Interactions \label{investigateinteractions}
...
We now shall select a linear model with all covariate and interaction terms. The summary of the resulting model can be found in the appendix \ref{modelsummary1}. 
...
The model selected by BIC here is still smaller than the one with AIC, and it also outperforms it. However, as this model is different from Model 1 but is still very parsimonious, we decided to consider this BIC model as our second candidate model, named Model 2.
...
By comparing the model performance on the testset, we observed that the model with the formula

```{r printbestmodelformula}
model2_f
``` 

This model is also the most parsimonious and interpretable. Thus we shall further analyze this model and draw conclusions from it.

# Results

We have built four models in the previous section and computed their performance scores on a test set. The MPSE of the four models we have considered are as follows:
...
We also decided in the last section that Model 2 has the best prediction performance, is the most interpretable and parsimonious. Thus, we shall now take a closer look at the parameters and coefficients of this model.

```{r coefbestmodel}
coef(cv_ridge_model2)
``` 

The output shows that Model 2 considers POP_furan3 and ageyrs and the coefficient values suggest that age and the PCB pollutant have a negative linear effect on average leukocyte telomere length. This implies that telomere length decreases with increasing age and increasing exposure to furan3. This result is intuitive as pollutants are harmful to human health thus accelerates telomere wear. 

As mentioned earlier, this model is generalized and unlikely to overfit, whic means we can use it to answer the questions in our objective section. From Model 2, we may conclude that both the age of a person and their exposure to furan 3 contribute greatly to the average leukocyte telomere length. 

Furthermore, upon investigation into the other three models (in appendix \ref{modelcoef}), we noticed that with all models, age is significant as a covariate, while the other factors are not consistent. Hence we may conclude that based on the given data, the age of the person is the main factor in telomere length, and exposure to different pollutants tends to reduce the length as well.


# Discussion

We know linear regression model has four assumptions, namely linearity, normality, homoscedasticity, and independence. We have analyzed and confirmed that the first three assumptions hold. Generally, we can assume independence when constructing the model. To further confirm the assumption, time-series data and a closer look at the data collection process will be helpful.

In section \ref{investigateinteractions}, we have considered the interactions within the eleven PCB covariates, three dioxin covariates, and four furan covariates with different white blood cell components. It is expected that there is no causal relationship between exposure covariates and other personal characteristics variables. For example, it is reasonable to assume that the concentration of POP_PCB10 is unrelated to the value of ageyrs and BMI. However, it may still be useful to confirm this hypothesis by p-tests.


In the result section, we have mentioned that age is always present as a covariate in our models, while other pollutants' effects are not always consistent. Thus it may be interesting to investigate linear models with age being the only covariate, as well as with removed from the dataset. The first shall tell us how the exact effect of age on telomere, while the latter shall provide insights into which human-adjustable factors have the most significant to telomere length in a person.
...
## Results Model Coefficients \label{modelcoef}
```{r coef4all}
