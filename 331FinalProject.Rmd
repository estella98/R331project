...
``` 
This showed that the model selected by BIC was prefered. it gave us a lower MSE and it was also simple enough to interpret. At the same time, through
the comparisons, we concluded that the interaction terms we filtered out earlier were unnecessary in our model. In the end, one additional technique we used to reduce the influence of multicollinearity is variance inflation factor(VIF). As interaction terms were analyzed to be ignorable, it was sufficient to regress on all non-categorical covariates and excluded the coviariate with the largest VIF one at a time until there were no more with 'high' multicollinearity. We decided to consider a VIF (Variance Inflation Factor) > 10 as an indicator of "high" multicollinearity. And after the covariate eliminations, the reduced covariates looked like: 
...
system.time({
  MAIC_Interaction <- step(object = Mfull,
                           scope = list(lower = M0, upper = Mfull),
                           direction = "both", trace = 0, k = 2)
})
...
#stepwiseBIC
system.time({
  MBIC_Interaction <- step(object = Mfull,
                           scope = list(lower = M0, upper = Mfull),
                           direction = "both", trace = 0, 
                           k = log(nrow(train_data)))
})
...
mean((test_data$length - predBICInteraction)^2)
``` 
As we see, we get the same result that the model selected by BIC still outperformed the one selected by AIC. Therefore this model is our first candidate model I. We also built another model using the automatic parameter selections of Lasso, which we would discuss later.


In order to get accurate prediction evaluations for our models, we used the idea of 80% and 20% train-test split; To ensure the entire training set is covered and each observation is well represented, we divided the training data into 10 folds and repeatedly cross-validated the MSPE. Besides, we performed shrinkage methods like lasso and ridge to solve the overfitting problem. For example, we used ridge with cross validation to update our Model I as follow: 
...
BIC_train_X <-  train_data[, c( "ageyrs", "POP_furan3")] # Features selected by BIC 
BIC_test_X <-  test_data[, c( "ageyrs", "POP_furan3")] 
 # use ridge, default 10 folds
cvfit_ridge <- cv.glmnet(x = data.matrix(BIC_train_X ), y = Y, alpha = 0)


#estimated betas for min lambda
coef(cvfit_ridge, s = "lambda.min")
pred_ridge_BIC <- predict(cvfit_ridge, newx = data.matrix(BIC_test_X), s = "lambda.min")

```
With the consideration that lasso could also do parameter selections, we examined sending the remaining covariates in the VIF reduced model alongwith the categorical covariates to ‘glmnet’ function and let it pick the best model for us. 
...
library(data.table)
library(mltools)
train_df <- as.data.table(train_data[c(VIFselected,"edu_cat", "race_cat", "smokenow", "male")])
train_oh <- one_hot(train_df )
test_df <- as.data.table(test_data[c(VIFselected,"edu_cat", "race_cat", "smokenow", "male")])
...
#model 1
mean((test_data$length - pred_ridge_BIC)^2)
#model 2
mean((test_data$length - pred_lasso)^2)
