...
```
 ...
 We observe no interaction should be included for Pop_furan and Popl_dioxin，and we only need to those interactions in POP_PCB:
"POP_PCB2:POP_PCB6"   "POP_PCB2:POP_PCB7"   "POP_PCB2:POP_PCB8"   "POP_PCB2:POP_PCB10"  "POP_PCB3:POP_PCB7"  
"POP_PCB6:POP_PCB11"  "POP_PCB7:POP_PCB8"   "POP_PCB10:POP_PCB11"
...
BIC_train_X <-  train_data[, c("POP_furan3", "ageyrs")] # Features selected by BIC without interactions
BIC_test_X <-  test_data[, c("POP_furan3", "ageyrs")] 

cvfit_ridge <- cv.glmnet(x = data.matrix(BIC_train_X ), y = Y, alpha = 0) # use ridge
# plot(cvfit_lasso)
...
```
!!! suspicious, this value is higher than the mse without cross validation and ridge 


----- !!!need to investigate the behavior of glmnet, whether we need to manually do one hot encoding.
------ check the two chunks below for difference 
```{r}
...
# cvfit_lasso <- cv.glmnet(x = data.matrix(), y = Y, alpha = 0) # use ridge

``` 

```{r}
# try lasso and let lasso do the parameters selection
all_train_X <- train_data[, names(train_data) != "length"]
cvfit_lasso <- cv.glmnet(x = data.matrix(all_train_X), y = Y, alpha = 1) # use lasso
