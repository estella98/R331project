...
We also decided in the last section that Model 2 has the best prediction performance, is the most interpretable and parsimonious. We shall now take a closer look at the parameters and coefficients of this model.
...
As mentioned earlier, this model is generalized and unlikely to overfit. We can now answer the questions in our objective. From Model 2, we may conclude that both the age of a person and their exposure to furan 3 contribute greatly to the average leukocyte telomere length. 

Furthermore, upon investigation into the other three models (in appendix \ref{modelcoef}), we noticed that with all models, when age is present as a covariate, while the other factors are not consistent. We shall further discuss this in the Discussion section below. Thus we may conclude that in general, the age of the person is the main factor in telomere length, and exposure to different pollutants tends to reduce the length as well.
