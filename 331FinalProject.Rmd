...
```{r interactions}
# rewrite the pollutants dataframe
pollutants_clean <- subset(pollutants , select = -X )
```

```{r interactions}
#stepwise parameters selection without any interaction terms
M0 <- lm(length ~ 1, data = pollutants_clean) # minimal model
Mfull <- lm(length ~ .,
            data= pollutants_clean )
...
single <- paste(tail(colnames(pollutants_clean),-1), collapse = " + ")
#tail to remove intercept column
interaction <- paste(tail(names(selected),-1), collapse = " + ")
f_interaction <- as.formula((paste("length", paste("(", single,"+", interaction, ")"), sep = " ~")))
Mfull <- lm(f_interaction, data = pollutants_clean)
Mstart <- lm(f_interaction, data = pollutants_clean)
