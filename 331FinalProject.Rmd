...
```
```{r testmodels}
covariates_AIC <- names(MAIC_Interaction$coefficients[2:length(MAIC_Interaction$coefficients)])
covariates_BIC <- names(MBIC_Interaction$coefficients[2:length(MBIC_Interaction$coefficients)])
```
...
the comparisons, we concluded that the interaction terms we filtered out earlier were unnecessary in our model.
...
BIC_train_X <-  train_data[, covariates_BIC] # Features selected by BIC 
BIC_test_X <-  test_data[, covariates_BIC] 
...
```
```{r}
## estella's work cross validation using lasso to do model selections
## model 2
# Load libraries
library(data.table)
library(mltools)
train_df <- as.data.table(train_data)
train_oh <- one_hot(train_df )
test_df <- as.data.table(test_data)
test_oh <- one_hot(test_df )

# try lasso and let lasso do the parameters selection
all_train_oh_X <- train_oh[, -1] # remove y 
all_test_oh_X <- test_oh[, -1] # remove y 
cvfit_lasso_oh <- cv.glmnet(x = data.matrix(all_train_oh_X), y = Y, alpha = 1) # use lasso
coef(cvfit_lasso_oh, s = "lambda.min")
pred_lasso_all <- predict(cvfit_lasso_oh, newx = data.matrix(all_test_oh_X), s = "lambda.min")
mean((test_oh$length - pred_lasso_all)^2)
```
